...
> [!WARNING]
> **Azure Active Directory (Azure AD) Graph está en desuso.** Para evitar la pérdida de funcionalidad, migre las aplicaciones en la nube pública de Azure a Microsoft Graph antes del 30 de junio de 2022, cuando los puntos de conexión de la API Azure AD Graph dejarán de responder a las solicitudes.
>
> Microsoft continuará con el soporte técnico y aplicará correcciones de seguridad Azure AD Graph hasta el 30 de junio de 2022, cuando finalicen todas las funciones y soporte técnico. Si no migra las aplicaciones a Microsoft Graph antes del 30 de junio de 2022, pone en riesgo su funcionalidad y estabilidad.
> 
>**Actualiza tus Azure AD Graph aplicaciones para usar Microsoft Graph ahora.**


Para completar el movimiento de la nube de Azure German a la nube pública de Azure, recomendamos que el punto de conexión de autenticación, Azure Active Directory (Azure AD) Graph y los puntos de conexión MS Graph para las aplicaciones se actualicen a los de la nube comercial cuando el punto de conexión de OpenID Conectar (OIDC), comience a notificar `https://login.microsoftonline.com/\<TenantIdOrDomain\>/.well-known/openid-configuration` co Puntos de conexión de nube mmerciales. 
...
Existen tres condiciones previas para actualizar la entidad de inicio de sesión:
 ...
 - El punto de conexión de detección de OIDC para el `https://login.microsoftonline.com/<TenantIdOrDomain>/.well-known/openid-configuration` inquilino devuelve Azure AD de nube pública.

 - Si el inquilino está configurado para la federación, los Servicios de federación de Active Directory (AD FS) se actualizan para sincronizarse con Azure AD Public. Puedes seguir las instrucciones para actualizar la Azure AD Conectar configuración para realizar este cambio.

 - Las aplicaciones de recursos, si las hay, se modifican para aceptar Azure AD tokens firmados tanto por alemania como por Azure AD público.
...
> [!NOTE] 
> Cuando una aplicación pasa a `login.microsoftonline.com` usarse como autoridad, los tokens se firmarán con esta nueva autoridad. Si hospedas cualquier aplicación de recursos a la que llamen otras aplicaciones, deberás permitir la validación de tokens lax. Esto significa que la aplicación debe permitir tokens firmados por la Azure AD Alemania y Azure AD nubes públicas. Esta validación de token lax es necesaria hasta que todas las aplicaciones cliente que llaman al servicio se migren completamente a la Azure AD nube pública. Después de la migración, la aplicación de recursos solo debe aceptar tokens firmados por la Azure AD nube pública.
...
1. Si hospeda una aplicación en Azure Germany que se usa para autenticar usuarios de Azure Germany o Office 365 Alemania, asegúrese de que se usa como autoridad en el contexto `https://login.microsoftonline.com` de autenticación.
    ...
    - Consulte Azure AD contextos de autenticación.
...
2. Actualizar Azure AD Graph punto de conexión para que sea `https://graph.windows.net` .
...
- Para redes:
  ...
  - La creación de redes con nombre IPv6 no funciona en Azure Portal, `http://portal.microsoftazure.de/` . Use Azure Portal at `https://portal.azure.com` para crear redes con nombre IPv6.
   ...
   - No puede crear intervalos de direcciones IP de confianza para la configuración del servicio Azure Multi-Factor Authentication (MFA) desde el portal de Microsoft Cloud Deutschland. Use el portal Azure AD para Office 365 para crear intervalos de direcciones IP de confianza de Azure MFA.


- Para acceso condicional: 

  - Las directivas de acceso condicional con los siguientes controles de concesión no se admiten hasta que se complete la migración a Office 365 servicios (después de finalizar [Azure AD](ms-cloud-germany-transition.md#how-is-the-migration-organized) de migración):
...
- Los usuarios de Microsoft Cloud Deutschland que usan el método Mobile App Notification para solicitudes MFA ven el ObjectId (un GUID) del usuario en lugar del nombre principal de usuario (UPN) en la aplicación Microsoft Authenticator. Una vez completada la migración del espacio empresarial Azure AD y hospedado en servicios Office 365, las nuevas Microsoft Authenticator activaciones mostrarán los UPN de los usuarios. Las Microsoft Authenticator existentes seguirán mostrar el ObjectId del usuario, pero seguirán funcionando para las notificaciones de aplicaciones móviles. 
