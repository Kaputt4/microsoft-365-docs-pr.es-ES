---
title: Implementar Enterprise del 10 de Windows para los nuevos dispositivos con piloto automático de Windows
description: Proporciona orientación sobre configuración e implementación de Windows 10 Enterprise con piloto automático de Windows.
keywords: Implementación de Microsoft 365, 365 Enterprise de Microsoft, Microsoft 365 documentación, Windows 10 Enterprise, piloto automático de Windows
author: greg-lindsay
...
ms.locfileid: "26871409"
---
# <a name="step-3-deploy-windows-10-enterprise-for-new-devices-with-windows-autopilot"></a><span data-ttu-id="8f05c-104">Paso 3: Implementar Windows 10 Enterprise para nuevos dispositivos con piloto automático de Windows</span><span class="sxs-lookup"><span data-stu-id="8f05c-104">Step 3: Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>

<span data-ttu-id="8f05c-105">*En este artículo se aplica a la E3 y E5 versiones de Microsoft 365 Enterprise*</span><span class="sxs-lookup"><span data-stu-id="8f05c-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="8f05c-p101">Si dispone de equipos nuevos de 10 de Windows, puede usar el piloto automático de Windows para personalizar fuera de cuadro de la experiencia (OOBE) para la organización e implementar un nuevo sistema con aplicaciones y configuración que se haya configurado. Hay no hay imágenes para implementar, no hay controladores para insertar y sin infraestructura para administrar. Los usuarios pueden ir a través del proceso de implementación de forma independiente, sin necesidad de consultar a su administrador de TI.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p101">If you have new Windows 10 PCs, you can use Windows Autopilot to customize the out-of-box-experience (OOBE) for your organization and deploy a new system with apps and settings already configured. There are no images to deploy, no drivers to inject, and no infrastructure to manage. Users can go through the deployment process independently, without the need consult their IT administrator.</span></span>

<span data-ttu-id="8f05c-p102">Puede configurar y configurar previamente los nuevos dispositivos de Windows 10 y que estén listos para su uso productivo uso piloto automático de Windows. Para obtener más información acerca de piloto automático de Windows, incluidos los beneficios y escenarios de piloto automático de Windows, vea [Información general de piloto automático de Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). Cuando esté listo, siga estos elementos para iniciar la configuración de dispositivos nuevos.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p102">You can set up and pre-configure new Windows 10 devices and get them ready for productive use using Windows Autopilot. To learn more about Windows Autopilot, including benefits and Windows Autopilot scenarios, see [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). When ready, follow these parts to start setting up new devices.</span></span>

## <a name="part-1-start-windows-autopilot-deployment"></a><span data-ttu-id="8f05c-112">Parte 1: Implementación de piloto automático de Windows de inicio</span><span class="sxs-lookup"><span data-stu-id="8f05c-112">Part 1: Start Windows Autopilot deployment</span></span>
<span data-ttu-id="8f05c-113">Vea [información general de piloto automático de Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) para:</span><span class="sxs-lookup"><span data-stu-id="8f05c-113">See [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) to:</span></span>

1. <span data-ttu-id="8f05c-p103">Obtenga información sobre y completar los requisitos previos para la implementación piloto automático de Windows. Los requisitos previos incluyen:</span><span class="sxs-lookup"><span data-stu-id="8f05c-p103">Learn about and complete the prerequisites for Windows Autopilot deployment. The prerequisites include:</span></span>
    - <span data-ttu-id="8f05c-116">**El registro de dispositivos y la personalización de la configuración rápida**</span><span class="sxs-lookup"><span data-stu-id="8f05c-116">**Device registration and OOBE customization**</span></span>

        <span data-ttu-id="8f05c-p104">Para registrar los dispositivos, debe adquirir su identificador de hardware y registrarlo. Estamos trabajando activamente con varios proveedores de hardware para que puedan proporcionar la información necesaria para usted o cargar en su nombre. También tiene la opción para capturar esta información por usted mismo utilizando un script de PowerShell que genera un archivo .csv con el identificador de hardware. del dispositivo</span><span class="sxs-lookup"><span data-stu-id="8f05c-p104">To register devices, you need to acquire their hardware ID and register it. We are actively working with various hardware vendors to enable them to provide the required information to you, or upload it on your behalf. You also have the option to capture this information by yourself using a PowerShell script that generates a .csv file with the device's hardware ID.</span></span>

        <span data-ttu-id="8f05c-120">Una vez que los dispositivos están registrados, existen opciones de personalización de OOBE que puede configurar incluida la omisión de la configuración de privacidad y los términos de licencia.</span><span class="sxs-lookup"><span data-stu-id="8f05c-120">Once devices are registered, there are OOBE customization options that you can configure including skipping privacy settings and EULA.</span></span>

    - <span data-ttu-id="8f05c-121">**Personalización de marca para configuración rápida**</span><span class="sxs-lookup"><span data-stu-id="8f05c-121">**Company branding for OOBE**</span></span>

        <span data-ttu-id="8f05c-122">Esto permite agregar personalización de marca para que aparezca durante OOBE del dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8f05c-122">This allows you to add branding to appear during device OOBE.</span></span>

    - <span data-ttu-id="8f05c-123">**Inscripción automática de MDM en Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="8f05c-123">**MDM auto-enrollment in Microsoft Intune**</span></span>
        
        <span data-ttu-id="8f05c-p105">La inscripción automática permite a los usuarios inscribirse sus dispositivos de Windows 10 en Intune para administración de dispositivos al conectarse con sus dispositivos a Azure AD. Para inscribirse, los usuarios agregar su cuenta de trabajo en los dispositivos de sus propiedad de los usuarios o unirse a dispositivos corporativo que pertenecen a Azure AD. En segundo plano, el dispositivo también está inscrito de la administración con Intune.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p105">Automatic enrollment lets users enroll their Windows 10 devices in Intune for device management when they connect their devices to Azure AD. To enroll, users add their work account to their personally-owned devices or join corporate-owned devices to Azure AD. In the background, the device is also enrolled for management with Intune.</span></span>

    - <span data-ttu-id="8f05c-127">**Conectividad de red a servicios en la nube usados por Windows Autopilot**</span><span class="sxs-lookup"><span data-stu-id="8f05c-127">**Network connectivity to cloud services used by Windows Autopilot**</span></span>

        <span data-ttu-id="8f05c-128">El programa de implementación piloto automático de Windows utiliza un número de servicios de nube para obtener los dispositivos a un estado productivo y estos servicios deben ser accesibles desde dispositivos registrados como dispositivos de piloto automático de Windows.</span><span class="sxs-lookup"><span data-stu-id="8f05c-128">The Windows Autopilot Deployment Program uses a number of cloud services to get your devices to a productive state and these services must be accessible from devices registered as Windows Autopilot devices.</span></span> 

    - <span data-ttu-id="8f05c-129">**Los dispositivos deben tener preinstalado Windows 10, versión 1703 o posterior**</span><span class="sxs-lookup"><span data-stu-id="8f05c-129">**Devices must be pre-installed with Windows 10, version 1703 or later**</span></span>

2. <span data-ttu-id="8f05c-p106">Obtenga información sobre y seleccione el programa de implementación piloto automático de Windows para su organización. Puede seleccionar entre estos programas de implementación:</span><span class="sxs-lookup"><span data-stu-id="8f05c-p106">Learn about and select the Windows Autopilot Deployment Program for your organization. You can select from these deployment programs:</span></span>
    - <span data-ttu-id="8f05c-132">**Microsoft Store para Empresas**</span><span class="sxs-lookup"><span data-stu-id="8f05c-132">**Microsoft Store for Business**</span></span>
    - <span data-ttu-id="8f05c-133">**Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="8f05c-133">**Microsoft Intune**</span></span>
    - <span data-ttu-id="8f05c-134">**Centro para socios**</span><span class="sxs-lookup"><span data-stu-id="8f05c-134">**Partner Center**</span></span>

## <a name="part-2-set-up-a-windows-10-device-for-microsoft-365"></a><span data-ttu-id="8f05c-135">Parte 2: Al configurar un dispositivo de Windows 10 para Microsoft 365</span><span class="sxs-lookup"><span data-stu-id="8f05c-135">Part 2: Set up a Windows 10 device for Microsoft 365</span></span>
<span data-ttu-id="8f05c-136">Antes de configurar los dispositivos de Windows para los usuarios de Microsoft 365, asegúrese de que todos los dispositivos de Windows ejecutan Windows 10, versión 1703 (los creadores de actualización) o posterior.</span><span class="sxs-lookup"><span data-stu-id="8f05c-136">Before you can set up Windows devices for Microsoft 365 users, make sure all the Windows devices are running Windows 10, version 1703 (Creators Update) or later.</span></span>

<span data-ttu-id="8f05c-137">Después de que todos los dispositivos de Windows de la organización o bien se han actualizado a los creadores de 10 Windows Update o ya se está ejecutando Windows 10 los creadores de actualización, puede participar en estos dispositivos a Azure Active Directory de su organización.</span><span class="sxs-lookup"><span data-stu-id="8f05c-137">After all Windows devices in your organization have either been upgraded to Windows 10 Creators Update or are already running Windows 10 Creators Update, you can join these devices to your organization’s Azure Active Directory.</span></span>

### <a name="set-up-a-brand-new-or-newly-upgraded-windows-10-device"></a><span data-ttu-id="8f05c-138">Configurar un dispositivo de Windows 10 totalmente nuevo o actualizado recientemente</span><span class="sxs-lookup"><span data-stu-id="8f05c-138">Set up a brand new or newly-upgraded Windows 10 device</span></span>
<span data-ttu-id="8f05c-139">Siga estos pasos para configurar un dispositivo con el 10 de Windows OOBE en un dispositivo nuevo que ejecuta Windows Update de los creadores de 10 (o posterior) o en un dispositivo que se actualice a Windows Update de los creadores de 10 (o posterior), pero no se ha pasado por el programa de instalación de fábrica.</span><span class="sxs-lookup"><span data-stu-id="8f05c-139">Follow these steps to set up a device using the Windows 10 OOBE on a brand new device running Windows 10 Creators Update (or later) or on a device that was upgrade to Windows 10 Creators Update (or later) but has not gone through out-of-box setup.</span></span>

1. <span data-ttu-id="8f05c-140">Si no tiene configurada una red inalámbrica, asegúrese de que el dispositivo se conecta a internet a través de una por cable o una conexión Ethernet.</span><span class="sxs-lookup"><span data-stu-id="8f05c-140">If you don't have a wireless network configured, make sure you connect the device to the internet through a wired or Ethernet connection.</span></span>
2. <span data-ttu-id="8f05c-p107">Vaya a través de la experiencia del programa de instalación de dispositivo de Windows. En un dispositivo nuevo o restablecer, la experiencia del programa de instalación inicia con el **vamos a inicio con región. Es este derecho?** pantalla.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p107">Go through the Windows device setup experience. On a new or reset device, the setup experience starts with the **Let's start with region. Is this right?** screen.</span></span>
3. <span data-ttu-id="8f05c-p108">Vaya a través de configuración de dispositivo de Windows 10 hasta llegar a la **¿cómo desea configurar?** página. A continuación, seleccione **configurar para una organización**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p108">Go through Windows 10 device setup until you get to the **How would you like to set up?** page. Here, select **Set up for an organization**.</span></span>
4. <span data-ttu-id="8f05c-p109">Iniciar sesión mediante la cuenta y la contraseña del usuario de Microsoft 365. Dependiendo de la configuración de contraseña de usuario, es posible que se le pida para actualizar la contraseña.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p109">Sign in using the Microsoft 365 user's account and password. Depending on the user password setting, you may be prompted to update the password.</span></span> 
5. <span data-ttu-id="8f05c-147">Finalice la configuración del dispositivo Windows 10.</span><span class="sxs-lookup"><span data-stu-id="8f05c-147">Finish Windows 10 device setup.</span></span>

<span data-ttu-id="8f05c-148">Una vez haya terminado, el dispositivo se conectará a Azure la organización AD.</span><span class="sxs-lookup"><span data-stu-id="8f05c-148">After you’re done, the device will be connected to your organization’s Azure AD.</span></span>

### <a name="set-up-a-device-that-has-already-completed-out-of-box-setup"></a><span data-ttu-id="8f05c-149">Configurar un dispositivo que ya se ha completado el programa de instalación de fábrica</span><span class="sxs-lookup"><span data-stu-id="8f05c-149">Set up a device that has already completed out-of-box setup</span></span>
<span data-ttu-id="8f05c-150">Si su dispositivo tiene Windows 10 los creadores de actualización (o posterior) y tiene ya se ha pasado a través de la configuración de fábrica, siga estos pasos.</span><span class="sxs-lookup"><span data-stu-id="8f05c-150">If your device has Windows 10 Creators Update (or later) and has already gone through the out-of-box setup, follow these steps.</span></span>

1. <span data-ttu-id="8f05c-151">En los PC de Windows del usuario que está ejecutando Windows 10, versión 1703 (los creadores de actualización), seleccione el logotipo de **Windows** y, a continuación, seleccione el icono de **configuración** .</span><span class="sxs-lookup"><span data-stu-id="8f05c-151">On your user's Windows PC that is running Windows 10, version 1703 (Creators Update), select the **Windows** logo, and then select the **Settings** icon.</span></span>
2. <span data-ttu-id="8f05c-152">En **Configuración**, vaya a **Cuentas**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-152">In **Settings**, go to **Accounts**.</span></span>
3. <span data-ttu-id="8f05c-153">En la página de **la información** , seleccione **el trabajo de acceso o la escuela** > **Conectar**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-153">On the **Your info** page, select **Access work or school** > **Connect**.</span></span>
4. <span data-ttu-id="8f05c-154">En el cuadro de diálogo **Configurar una cuenta de trabajo o escuela** , en **acciones alternativas**, seleccione **unirse a este dispositivo a Azure Active Directory**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-154">On the **Set up a work or school account** dialog, under **Alternate actions**, select **Join this device to Azure Active Directory**.</span></span>
5. <span data-ttu-id="8f05c-155">En la página **vamos a obtener que se ha registrado** , escriba su trabajo o escuela cuenta y seleccione **siguiente**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-155">On the **Let's get you signed in** page, enter your work or school account, and select **Next**.</span></span>
6. <span data-ttu-id="8f05c-156">En la página **Escriba la contraseña** , escriba su contraseña y seleccione **iniciar sesión**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-156">On the **Enter password** page, enter your password, and select **Sign in**.</span></span>
7. <span data-ttu-id="8f05c-157">En la página **Asegúrese de que se trata de la organización** , compruebe que la información es correcta y, a continuación, seleccione **unirse**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-157">On the **Make sure this is your organization** page, verify that the information is correct, and select **Join**.</span></span>
8. <span data-ttu-id="8f05c-p110">En la **está todos los conjunto!** página, seleccione **hecho**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p110">On the **You're all set!** page, select **Done**.</span></span>

<span data-ttu-id="8f05c-160">Una vez haya terminado, el usuario se conectará a Azure la organización AD.</span><span class="sxs-lookup"><span data-stu-id="8f05c-160">After you're done, the user will be connected to your organization's Azure AD.</span></span>

### <a name="verify-the-device-is-connected-to-azure-ad"></a><span data-ttu-id="8f05c-161">Comprobar que un dispositivo está conectado a Azure AD</span><span class="sxs-lookup"><span data-stu-id="8f05c-161">Verify the device is connected to Azure AD</span></span>
<span data-ttu-id="8f05c-162">Siga estos pasos para comprobar el estado de la sincronización del dispositivo con Azure AD y, a continuación, empezar a usar su cuenta Microsoft 365 en el dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8f05c-162">Follow these steps to verify the device’s sync status with Azure AD, and then start using your Microsoft 365 account on the device.</span></span> 

1. <span data-ttu-id="8f05c-163">Abra **configuración**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-163">Open **Settings**.</span></span>
2. <span data-ttu-id="8f05c-164">En la página **trabajo de acceso o la escuela** , seleccione la \*\*conectado a <organization name> \*\* área para exponer los botones **Info** y **Desconectar**.</span><span class="sxs-lookup"><span data-stu-id="8f05c-164">On the **Access work or school** page, select the **Connected to <organization name>** area to expose the buttons **Info** and **Disconnect**.</span></span>
3. <span data-ttu-id="8f05c-165">Seleccione la **información** para obtener el estado de sincronización.</span><span class="sxs-lookup"><span data-stu-id="8f05c-165">Select **Info** to get your synchronization status.</span></span>
4. <span data-ttu-id="8f05c-166">En la página **estado de la sincronización** , seleccione **sincronización** para obtener el dispositivo móvil más reciente de las directivas de administración en el PC.</span><span class="sxs-lookup"><span data-stu-id="8f05c-166">On the **Sync status** page, select **Sync** to get the latest mobile device management policies onto the PC.</span></span>
5. <span data-ttu-id="8f05c-167">Para empezar a usar la cuenta de Microsoft 365, vaya al botón **Inicio** de Windows, haga clic en la imagen de la cuenta actual y, a continuación, seleccione cuenta de **modificador** .</span><span class="sxs-lookup"><span data-stu-id="8f05c-167">To start using the Microsoft 365 account, go to the Windows **Start** button, right-click your current account picture and then select **Switch** account.</span></span>
6. <span data-ttu-id="8f05c-168">Inicie sesión con el correo electrónico y la contraseña de la organización.</span><span class="sxs-lookup"><span data-stu-id="8f05c-168">Sign in by using your organization email and password.</span></span>

<span data-ttu-id="8f05c-p111">Si experimenta problemas con el 10 de Windows en un entorno empresarial, puede consultar [las mejores soluciones de soporte técnico de Microsoft para los problemas más comunes](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). Estos recursos incluyen artículos KB, actualizaciones y artículos de la biblioteca.</span><span class="sxs-lookup"><span data-stu-id="8f05c-p111">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="8f05c-171">Como control provisional, puede consultar los [criterios de salida](windows10-exit-criteria.md#crit-windows10-step3) correspondientes a este paso.</span><span class="sxs-lookup"><span data-stu-id="8f05c-171">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step3) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="8f05c-172">Paso siguiente</span><span class="sxs-lookup"><span data-stu-id="8f05c-172">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step4.png)| [<span data-ttu-id="8f05c-173">Utilice análisis de Windows para supervisar el estado de dispositivo</span><span class="sxs-lookup"><span data-stu-id="8f05c-173">Use Windows Analytics to monitor device health</span></span>](windows10-enable-windows-analytics.md) |
