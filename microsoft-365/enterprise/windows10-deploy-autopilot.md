...
---
# <a name="step-3-deploy-windows-10-enterprise-for-new-devices-with-windows-autopilot"></a><span data-ttu-id="8b52a-104">Paso 3: implementar Windows 10 Enterprise para dispositivos nuevos con Windows AutoPilot</span><span class="sxs-lookup"><span data-stu-id="8b52a-104">Step 3: Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>

<span data-ttu-id="8b52a-105">*Este artículo se aplica a las versiones E3 y E5 de Microsoft 365 Enterprise*</span><span class="sxs-lookup"><span data-stu-id="8b52a-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="8b52a-106">Si tienes nuevos equipos con Windows 10, puedes usar Windows AutoPilot para personalizar la experiencia rápida (OOBE) de tu organización e implementar un nuevo sistema con aplicaciones y configuración ya configurada.</span><span class="sxs-lookup"><span data-stu-id="8b52a-106">If you have new Windows 10 PCs, you can use Windows Autopilot to customize the out-of-box-experience (OOBE) for your organization and deploy a new system with apps and settings already configured.</span></span> <span data-ttu-id="8b52a-107">No hay imágenes para implementar, no hay controladores para insertar y no hay infraestructura que administrar.</span><span class="sxs-lookup"><span data-stu-id="8b52a-107">There are no images to deploy, no drivers to inject, and no infrastructure to manage.</span></span> <span data-ttu-id="8b52a-108">Los usuarios pueden pasar por el proceso de implementación de manera independiente, sin necesidad de consultar a su administrador de ti.</span><span class="sxs-lookup"><span data-stu-id="8b52a-108">Users can go through the deployment process independently, without the need consult their IT administrator.</span></span>

<span data-ttu-id="8b52a-109">Puede configurar y preconfigurar nuevos dispositivos Windows 10 y prepararlos para un uso productivo mediante Windows AutoPilot.</span><span class="sxs-lookup"><span data-stu-id="8b52a-109">You can set up and pre-configure new Windows 10 devices and get them ready for productive use using Windows Autopilot.</span></span> <span data-ttu-id="8b52a-110">Para obtener más información acerca de Windows AutoPilot, incluidas las ventajas y los escenarios de Windows AutoPilot, consulte [Overview of Windows AutoPilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot).</span><span class="sxs-lookup"><span data-stu-id="8b52a-110">To learn more about Windows Autopilot, including benefits and Windows Autopilot scenarios, see [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot).</span></span> <span data-ttu-id="8b52a-111">Cuando esté listo, siga estas partes para empezar a configurar nuevos dispositivos.</span><span class="sxs-lookup"><span data-stu-id="8b52a-111">When ready, follow these parts to start setting up new devices.</span></span>

## <a name="part-1-start-windows-autopilot-deployment"></a><span data-ttu-id="8b52a-112">Parte 1: iniciar la implementación de Windows AutoPilot</span><span class="sxs-lookup"><span data-stu-id="8b52a-112">Part 1: Start Windows Autopilot deployment</span></span>
<span data-ttu-id="8b52a-113">Vea [información general de Windows AutoPilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) para:</span><span class="sxs-lookup"><span data-stu-id="8b52a-113">See [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) to:</span></span>

1. <span data-ttu-id="8b52a-114">Obtenga información y cómo completar los requisitos previos para la implementación de Windows AutoPilot.</span><span class="sxs-lookup"><span data-stu-id="8b52a-114">Learn about and complete the prerequisites for Windows Autopilot deployment.</span></span> <span data-ttu-id="8b52a-115">Entre los requisitos previos se incluyen:</span><span class="sxs-lookup"><span data-stu-id="8b52a-115">The prerequisites include:</span></span>
    - <span data-ttu-id="8b52a-116">**El registro de dispositivos y la personalización de la configuración rápida**</span><span class="sxs-lookup"><span data-stu-id="8b52a-116">**Device registration and OOBE customization**</span></span>

        <span data-ttu-id="8b52a-117">Para registrar dispositivos, debe adquirir su identificador de hardware y registrarlo.</span><span class="sxs-lookup"><span data-stu-id="8b52a-117">To register devices, you need to acquire their hardware ID and register it.</span></span> <span data-ttu-id="8b52a-118">Estamos trabajando activamente con varios proveedores de hardware para que puedan proporcionarle la información necesaria o cargarlo en su nombre.</span><span class="sxs-lookup"><span data-stu-id="8b52a-118">We are actively working with various hardware vendors to enable them to provide the required information to you, or upload it on your behalf.</span></span> <span data-ttu-id="8b52a-119">También tiene la opción de capturar esta información mediante un script de PowerShell que genera un archivo. csv con el identificador de hardware del dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8b52a-119">You also have the option to capture this information by yourself using a PowerShell script that generates a .csv file with the device's hardware ID.</span></span>

        <span data-ttu-id="8b52a-120">Una vez que se han registrado los dispositivos, hay opciones de personalización de OOBE que puede configurar, lo que permite omitir la configuración de privacidad y el CLUF.</span><span class="sxs-lookup"><span data-stu-id="8b52a-120">Once devices are registered, there are OOBE customization options that you can configure including skipping privacy settings and EULA.</span></span>

    - <span data-ttu-id="8b52a-121">**Personalización de marca para configuración rápida**</span><span class="sxs-lookup"><span data-stu-id="8b52a-121">**Company branding for OOBE**</span></span>

        <span data-ttu-id="8b52a-122">Esto le permite agregar personalización de marca para que aparezca durante la OOBE del dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8b52a-122">This allows you to add branding to appear during device OOBE.</span></span>

    - <span data-ttu-id="8b52a-123">**Inscripción automática de MDM en Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="8b52a-123">**MDM auto-enrollment in Microsoft Intune**</span></span>
        
        <span data-ttu-id="8b52a-124">La inscripción automática permite a los usuarios inscribir sus dispositivos Windows 10 en Intune para la administración de dispositivos cuando conectan sus dispositivos a Azure AD.</span><span class="sxs-lookup"><span data-stu-id="8b52a-124">Automatic enrollment lets users enroll their Windows 10 devices in Intune for device management when they connect their devices to Azure AD.</span></span> <span data-ttu-id="8b52a-125">Para inscribirse, los usuarios agregan su cuenta profesional a sus dispositivos de propiedad personal o se unen a los dispositivos de propiedad corporativa para Azure AD.</span><span class="sxs-lookup"><span data-stu-id="8b52a-125">To enroll, users add their work account to their personally-owned devices or join corporate-owned devices to Azure AD.</span></span> <span data-ttu-id="8b52a-126">En segundo plano, el dispositivo también está inscrito para la administración con Intune.</span><span class="sxs-lookup"><span data-stu-id="8b52a-126">In the background, the device is also enrolled for management with Intune.</span></span>

    - <span data-ttu-id="8b52a-127">**Conectividad de red a servicios en la nube usados por Windows Autopilot**</span><span class="sxs-lookup"><span data-stu-id="8b52a-127">**Network connectivity to cloud services used by Windows Autopilot**</span></span>

        <span data-ttu-id="8b52a-128">El programa de implementación de Windows AutoPilot usa una cantidad de servicios en la nube para obtener los dispositivos en un estado productivo y estos servicios deben ser accesibles desde dispositivos registrados como dispositivos de Windows AutoPilot.</span><span class="sxs-lookup"><span data-stu-id="8b52a-128">The Windows Autopilot Deployment Program uses a number of cloud services to get your devices to a productive state and these services must be accessible from devices registered as Windows Autopilot devices.</span></span> 

    - <span data-ttu-id="8b52a-129">**Los dispositivos deben tener preinstalado Windows 10, versión 1703 o posterior**</span><span class="sxs-lookup"><span data-stu-id="8b52a-129">**Devices must be pre-installed with Windows 10, version 1703 or later**</span></span>

2. <span data-ttu-id="8b52a-130">Obtenga información sobre y seleccione el programa de implementación de Windows AutoPilot para su organización.</span><span class="sxs-lookup"><span data-stu-id="8b52a-130">Learn about and select the Windows Autopilot Deployment Program for your organization.</span></span> <span data-ttu-id="8b52a-131">Puede seleccionar entre estos programas de implementación:</span><span class="sxs-lookup"><span data-stu-id="8b52a-131">You can select from these deployment programs:</span></span>
    - <span data-ttu-id="8b52a-132">**Microsoft Store para Empresas**</span><span class="sxs-lookup"><span data-stu-id="8b52a-132">**Microsoft Store for Business**</span></span>
    - <span data-ttu-id="8b52a-133">**Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="8b52a-133">**Microsoft Intune**</span></span>
    - <span data-ttu-id="8b52a-134">**Centro para socios**</span><span class="sxs-lookup"><span data-stu-id="8b52a-134">**Partner Center**</span></span>

## <a name="part-2-set-up-a-windows-10-device-for-microsoft-365"></a><span data-ttu-id="8b52a-135">Parte 2: configurar un dispositivo con Windows 10 para Microsoft 365</span><span class="sxs-lookup"><span data-stu-id="8b52a-135">Part 2: Set up a Windows 10 device for Microsoft 365</span></span>
<span data-ttu-id="8b52a-136">Para poder configurar dispositivos Windows para los usuarios de Microsoft 365, asegúrese de que todos los dispositivos Windows ejecutan Windows 10, versión 1703 (Creators Update) o posterior.</span><span class="sxs-lookup"><span data-stu-id="8b52a-136">Before you can set up Windows devices for Microsoft 365 users, make sure all the Windows devices are running Windows 10, version 1703 (Creators Update) or later.</span></span>

<span data-ttu-id="8b52a-137">Una vez que todos los dispositivos Windows de su organización hayan sido actualizados a Windows 10 Creators Update o que ya estén ejecutando Windows 10 Creators Update, puede unir estos dispositivos al Azure Active Directory de la organización.</span><span class="sxs-lookup"><span data-stu-id="8b52a-137">After all Windows devices in your organization have either been upgraded to Windows 10 Creators Update or are already running Windows 10 Creators Update, you can join these devices to your organization’s Azure Active Directory.</span></span>

### <a name="set-up-a-brand-new-or-newly-upgraded-windows-10-device"></a><span data-ttu-id="8b52a-138">Configurar un dispositivo de Windows 10 nuevo o con una marca recién actualizada</span><span class="sxs-lookup"><span data-stu-id="8b52a-138">Set up a brand new or newly-upgraded Windows 10 device</span></span>
<span data-ttu-id="8b52a-139">Siga estos pasos para configurar un dispositivo con Windows 10 OOBE en un dispositivo nuevo que ejecute Windows 10 Creators Update (o posterior) o en un dispositivo que se haya actualizado a Windows 10 Creators Update (o posterior), pero que no haya pasado por una configuración rápida.</span><span class="sxs-lookup"><span data-stu-id="8b52a-139">Follow these steps to set up a device using the Windows 10 OOBE on a brand new device running Windows 10 Creators Update (or later) or on a device that was upgrade to Windows 10 Creators Update (or later) but has not gone through out-of-box setup.</span></span>

1. <span data-ttu-id="8b52a-140">Si no tiene una red inalámbrica configurada, asegúrese de conectar el dispositivo a Internet a través de una conexión cableada o Ethernet.</span><span class="sxs-lookup"><span data-stu-id="8b52a-140">If you don't have a wireless network configured, make sure you connect the device to the internet through a wired or Ethernet connection.</span></span>
2. <span data-ttu-id="8b52a-141">Pasar por la experiencia de instalación de dispositivos de Windows.</span><span class="sxs-lookup"><span data-stu-id="8b52a-141">Go through the Windows device setup experience.</span></span> <span data-ttu-id="8b52a-142">En un dispositivo nuevo o de restablecimiento, la experiencia del programa de instalación comienza con la **región comencemos. ¿Es este el derecho?** Screen.</span><span class="sxs-lookup"><span data-stu-id="8b52a-142">On a new or reset device, the setup experience starts with the **Let's start with region. Is this right?** screen.</span></span>
3. <span data-ttu-id="8b52a-143">Siga el proceso de configuración de dispositivos Windows 10 hasta que llegue a la página **¿Cómo quiere realizar la configuración?**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-143">Go through Windows 10 device setup until you get to the **How would you like to set up?** page.</span></span> <span data-ttu-id="8b52a-144">Seleccione **configurar para una organización**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-144">Here, select **Set up for an organization**.</span></span>
4. <span data-ttu-id="8b52a-145">Inicie sesión con la cuenta y la contraseña del usuario de Microsoft 365.</span><span class="sxs-lookup"><span data-stu-id="8b52a-145">Sign in using the Microsoft 365 user's account and password.</span></span> <span data-ttu-id="8b52a-146">Según la configuración de contraseña de usuario, es posible que se le pida que actualice la contraseña.</span><span class="sxs-lookup"><span data-stu-id="8b52a-146">Depending on the user password setting, you may be prompted to update the password.</span></span> 
5. <span data-ttu-id="8b52a-147">Finalice la configuración del dispositivo Windows 10.</span><span class="sxs-lookup"><span data-stu-id="8b52a-147">Finish Windows 10 device setup.</span></span>

<span data-ttu-id="8b52a-148">Una vez que haya terminado, el dispositivo se conectará a Azure AD de la organización.</span><span class="sxs-lookup"><span data-stu-id="8b52a-148">After you’re done, the device will be connected to your organization’s Azure AD.</span></span>

### <a name="set-up-a-device-that-has-already-completed-out-of-box-setup"></a><span data-ttu-id="8b52a-149">Configurar un dispositivo que ya ha completado la configuración de instalación</span><span class="sxs-lookup"><span data-stu-id="8b52a-149">Set up a device that has already completed out-of-box setup</span></span>
<span data-ttu-id="8b52a-150">Si el dispositivo tiene Windows 10 Creators Update (o posterior) y ya ha pasado mediante la configuración preconfigurada, siga estos pasos.</span><span class="sxs-lookup"><span data-stu-id="8b52a-150">If your device has Windows 10 Creators Update (or later) and has already gone through the out-of-box setup, follow these steps.</span></span>

1. <span data-ttu-id="8b52a-151">En el PC Windows del usuario que ejecuta la versión 1703 de (Creators Update), seleccione el logotipo de **Windows** y, a continuación, seleccione el icono de **configuración** .</span><span class="sxs-lookup"><span data-stu-id="8b52a-151">On your user's Windows PC that is running Windows 10, version 1703 (Creators Update), select the **Windows** logo, and then select the **Settings** icon.</span></span>
2. <span data-ttu-id="8b52a-152">En **Configuración**, vaya a **Cuentas**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-152">In **Settings**, go to **Accounts**.</span></span>
3. <span data-ttu-id="8b52a-153">En la página **su información** , seleccione **Access Work o School** > **Connect**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-153">On the **Your info** page, select **Access work or school** > **Connect**.</span></span>
4. <span data-ttu-id="8b52a-154">En el cuadro de diálogo **configurar una cuenta profesional o educativa** , en **acciones alternativas**, seleccione **unir este dispositivo a Azure Active Directory**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-154">On the **Set up a work or school account** dialog, under **Alternate actions**, select **Join this device to Azure Active Directory**.</span></span>
5. <span data-ttu-id="8b52a-155">En la página **vamos a iniciar sesión** , escriba su cuenta profesional o educativa y seleccione **siguiente**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-155">On the **Let's get you signed in** page, enter your work or school account, and select **Next**.</span></span>
6. <span data-ttu-id="8b52a-156">En la página **escribir contraseña** , escriba la contraseña y seleccione **iniciar sesión**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-156">On the **Enter password** page, enter your password, and select **Sign in**.</span></span>
7. <span data-ttu-id="8b52a-157">En la página Asegúrese de que **es su organización** , compruebe que la información es correcta y seleccione **unirse**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-157">On the **Make sure this is your organization** page, verify that the information is correct, and select **Join**.</span></span>
8. <span data-ttu-id="8b52a-158">En la de **todo está juego.**</span><span class="sxs-lookup"><span data-stu-id="8b52a-158">On the **You're all set!**</span></span> <span data-ttu-id="8b52a-159">en la página, seleccione **listo**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-159">page, select **Done**.</span></span>

<span data-ttu-id="8b52a-160">Una vez que haya terminado, el usuario se conectará a Azure AD de la organización.</span><span class="sxs-lookup"><span data-stu-id="8b52a-160">After you're done, the user will be connected to your organization's Azure AD.</span></span>

### <a name="verify-the-device-is-connected-to-azure-ad"></a><span data-ttu-id="8b52a-161">Comprobar que un dispositivo está conectado a Azure AD</span><span class="sxs-lookup"><span data-stu-id="8b52a-161">Verify the device is connected to Azure AD</span></span>
<span data-ttu-id="8b52a-162">Siga estos pasos para comprobar el estado de sincronización del dispositivo con Azure AD y, a continuación, empiece a usar su cuenta de Microsoft 365 en el dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8b52a-162">Follow these steps to verify the device’s sync status with Azure AD, and then start using your Microsoft 365 account on the device.</span></span> 

1. <span data-ttu-id="8b52a-163">Abra **configuración**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-163">Open **Settings**.</span></span>
2. <span data-ttu-id="8b52a-164">En la **Página tener acceso a la escuela o el trabajo** , seleccione el área \*\*conectado a <organization name> \*\* para mostrar la **información** de los botones y **desconectar**.</span><span class="sxs-lookup"><span data-stu-id="8b52a-164">On the **Access work or school** page, select the **Connected to <organization name>** area to expose the buttons **Info** and **Disconnect**.</span></span>
3. <span data-ttu-id="8b52a-165">Seleccione **información** para obtener el estado de sincronización.</span><span class="sxs-lookup"><span data-stu-id="8b52a-165">Select **Info** to get your synchronization status.</span></span>
4. <span data-ttu-id="8b52a-166">En la página **Estado de sincronización** , seleccione **sincronizar** para obtener las directivas de administración de dispositivos móviles más recientes en el equipo.</span><span class="sxs-lookup"><span data-stu-id="8b52a-166">On the **Sync status** page, select **Sync** to get the latest mobile device management policies onto the PC.</span></span>
5. <span data-ttu-id="8b52a-167">Para empezar a usar la cuenta de Microsoft 365, vaya al botón **Inicio** de Windows, haga clic con el botón secundario en la imagen de cuenta actual y seleccione **cambiar** de cuenta.</span><span class="sxs-lookup"><span data-stu-id="8b52a-167">To start using the Microsoft 365 account, go to the Windows **Start** button, right-click your current account picture and then select **Switch** account.</span></span>
6. <span data-ttu-id="8b52a-168">Inicie sesión con el correo electrónico y la contraseña de la organización.</span><span class="sxs-lookup"><span data-stu-id="8b52a-168">Sign in by using your organization email and password.</span></span>

<span data-ttu-id="8b52a-169">Si experimenta problemas al usar Windows 10 en un entorno empresarial, puede consultar [las principales soluciones de soporte técnico de Microsoft para los problemas más comunes](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions).</span><span class="sxs-lookup"><span data-stu-id="8b52a-169">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions).</span></span> <span data-ttu-id="8b52a-170">Estos recursos incluyen artículos de Knowledge base, actualizaciones y artículos de bibliotecas.</span><span class="sxs-lookup"><span data-stu-id="8b52a-170">These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="8b52a-171">Como control provisional, puede consultar los [criterios de salida](windows10-exit-criteria.md#crit-windows10-step3) correspondientes a este paso.</span><span class="sxs-lookup"><span data-stu-id="8b52a-171">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step3) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="8b52a-172">Siguiente paso</span><span class="sxs-lookup"><span data-stu-id="8b52a-172">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step4.png)| [<span data-ttu-id="8b52a-173">Supervisar el estado y el cumplimiento del dispositivo</span><span class="sxs-lookup"><span data-stu-id="8b52a-173">Monitor device health and compliance</span></span>](windows10-enable-windows-analytics.md) |
