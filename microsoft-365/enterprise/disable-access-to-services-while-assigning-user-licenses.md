...
- **AccountSkuId** es una suscripción para su organización en \<OrganizationName> : \<Subscription> formato. Es el valor que proporcionó cuando se inscribió en \<OrganizationName> Microsoft 365 y es único para su organización. El \<Subscription> valor es para una suscripción específica. Por ejemplo, para litwareinc:ENTERPRISEPACK, el nombre de la organización es litwareinc y el nombre de suscripción es ENTERPRISEPACK (Office 365 Enterprise E3).
...
Tenga en cuenta el AccountSkuId de su suscripción de Microsoft 365 que contiene los usuarios que desea licenciar. Además, asegúrese de que hay suficientes licencias para asignar (resta **ConsumedUnits** de **ActiveUnits** ).
...
Desde la visualización de este comando, determine qué planes de servicio desea deshabilitar al asignar licencias a los usuarios.
...
Para un solo usuario, rellene el nombre principal del usuario de la cuenta de usuario, accountSkuId y la lista de planes de servicio para deshabilitar y quitar el texto explicativo y los \< and > caracteres. A continuación, ejecute los comandos resultantes en el símbolo del sistema de PowerShell.
...
- Crea un archivo CSV con todos los usuarios procesados y muestra su estado de licencia.
