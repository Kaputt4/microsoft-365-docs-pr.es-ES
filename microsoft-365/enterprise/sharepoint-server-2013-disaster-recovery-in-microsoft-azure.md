...
description: En este artículo se describe cómo usar Azure para crear un entorno de recuperación ante desastres para la granja de SharePoint local.
ms.openlocfilehash: 1b1951e70cfbecc0f6586e68d7142bc26fb6252f
ms.sourcegitcommit: e50c13d9be3ed05ecb156d497551acf2c9da9015
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 04/27/2022
ms.locfileid: "65077410"
---
# <a name="sharepoint-server-2013-disaster-recovery-in-microsoft-azure"></a>Recuperación ante desastres de SharePoint Server 2013 en Microsoft Azure
 ...
 Con Azure, puede crear un entorno de recuperación ante desastres para la granja de SharePoint local. En este artículo se describe cómo diseñar e implementar esta solución.
...
|---|---|
|Activa|Se aprovisiona, actualiza y ejecuta una granja completa en modo de espera.|
|Semiactiva|Se crea la granja de servidores y se ponen en marcha y actualizan las máquinas virtuales. <br/> La recuperación incluye adjuntar bases de datos de contenido, aprovisionar aplicaciones de servicio y rastrear contenido. <br/> La granja de servidores puede ser una versión reducida de la granja de servidores de producción y más adelante se puede escalar horizontalmente para atender a la base completa de usuarios.|
|Pasiva|La granja de servidores se ha creado por completo, pero las máquinas virtuales están detenidas. <br/> Para realizar el mantenimiento del entorno, hay que iniciar las máquinas virtuales de vez en cuando, revisar, actualizar y comprobar el entorno. <br/> En caso de desastre, inicie el entorno completo.|
...
Para obtener más información sobre las soluciones de recuperación ante desastres, consulte [High availability and disaster recovery concepts in SharePoint 2013](/SharePoint/administration/high-availability-and-disaster-recovery-concepts) y [Choose a disaster recovery strategy for SharePoint 2013](/SharePoint/administration/plan-for-disaster-recovery).
...
![Elementos de una SharePoint solución en espera activa en Azure.](../media/AZarch-AZWarmStndby.png)
...
|Elemento|Descripción|
|---|---|
|Sincronización de soluciones de granja personalizadas|Lo ideal es que la configuración de la granja de servidores de recuperación sea idéntica a la granja de servidores de producción. Puede trabajar con un consultor o un socio para evaluar si se replicarán las soluciones de granja personalizadas y si hay un proceso para mantener los dos entornos sincronizados.|
|Conexiones a orígenes de datos de locales|No sería práctico replicar las conexiones en sistemas de datos back-end, como conexiones de controlador de dominio de reserva (BDC) y orígenes de contenido de búsqueda.|
|Escenarios de restauración de búsqueda|Dado que las implementaciones del motor de búsqueda Enterprise Search tienden a ser únicas y bastante complejas, para restaurar la búsqueda desde las bases de datos es necesario realizar una mayor inversión. Puede trabajar con un consultor o un socio para identificar e implementar los escenarios de restauración de búsqueda que puede necesitar la organización.|
...
![Topología de una granja de SharePoint y una granja de servidores de recuperación en espera activa.](../media/AZarch-AZWarmStndby.png)
...
![Elementos de una solución de SharePoint en espera en frío en Azure.](../media/AZarch-AZColdStndby.png)
...
- [Servicios de replicación del Sistema de archivos distribuido (DFS)](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127250(v=ws.11))

- [Clústeres de conmutación por error de Windows Server (WSFC) con SQL Server](/sql/sql-server/failover-clusters/windows/windows-server-failover-clustering-wsfc-with-sql-server)
...
- [Realizar copias de seguridad y restaurar bases de datos de SQL Server](/sql/relational-databases/backup-restore/back-up-and-restore-of-sql-server-databases)

- [Instalación e implementación de la granja de servidores de SharePoint Server 2013](/SharePoint/install/installation-and-configuration-overview)
...
|Fase 3|Implementar Windows Active Directory y servicios de nombres de dominio en la red virtual de Azure|
|Fase 4|Implementar la granja de servidores de recuperación de SharePoint en Azure.|
|Fase 5|Configurar DFSR entre granjas.|
|Fase 6|Configurar el trasvase de registros a la granja de servidores de recuperación.|
|Fase 7|Validar las soluciones de conmutación por error y recuperación. Esto incluye los siguientes procedimientos y tecnologías: <br/> Detener el trasvase de registros <br/> Restaurar las copias de seguridad <br/> Rastrear el contenido <br/> Recuperar servicios <br/> Administrar los registros DNS|

## <a name="phase-1-design-the-disaster-recovery-environment"></a>Fase 1: Diseñar el entorno de recuperación ante desastres

Siga las instrucciones de [Arquitecturas de Microsoft Azure para SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md) para diseñar el entorno de recuperación ante desastres, incluida la granja de servidores de recuperación de SharePoint. Puede usar los gráficos del archivo [SharePoint Disaster Recovery Solution en Azure](https://go.microsoft.com/fwlink/p/?LinkId=392554) Visio para iniciar el proceso de diseño. Le recomendamos que diseñe el entorno completo antes comenzar a trabajar en el entorno de Azure.
...
Si le preocupa la alta disponibilidad de los registros, considere la posibilidad de adoptar un enfoque diferente, mediante el uso de [copia de seguridad de SQL Server y restauración con el servicio de almacenamiento de blobs de Azure](/sql/relational-databases/backup-restore/sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service). Se trata de una nueva característica de Azure que guarda los registros directamente en una dirección URL de almacenamiento de blobs. Esta solución no incluye instrucciones sobre cómo usar esta característica.
...
![Dos máquinas virtuales implementadas en la red virtual de Azure y la subred SharePoint Farm son controladores de dominio de réplica y servidores DNS.](../media/AZarch-HyADdomainConfig.png)
...
Antes de implementar Active Directory en Azure, lea [Directrices para implementar Windows Server Active Directory en máquinas virtuales de Azure](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100). Estas directrices le ayudarán a determinar si necesita una arquitectura diferente o unos valores de configuración distintos para la solución.

Para obtener instrucciones detalladas sobre cómo configurar un controlador de dominio en Azure, consulte [Instalación de un controlador de dominio réplica de Active Directory en Redes virtuales de Azure](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100).
...
Implemente la granja de servidores de SharePoint en la Red virtual según sus planes de diseño. Puede resultarle útil revisar el tema sobre [Planeación para SharePoint 2013 en Servicios de infraestructura de Azure](/previous-versions/azure/dn275958(v=azure.100)) antes de implementar los roles de SharePoint en Azure.
...
|Título|Descripción|
|---|---|
|[Replicación](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770278(v=ws.11))|Tema de TechNet sobre administración de DFS con vínculos para la replicación|
|[Replicación DFS: Guía de supervivencia](https://go.microsoft.com/fwlink/p/?LinkId=392737)|Sitio wiki con vínculos a información sobre DFS|
|[Replicación DFS: Preguntas más frecuentes](/previous-versions/windows/it-pro/windows-server-2003/cc773238(v=ws.10))|Tema de TechNet sobre replicación DFS|
|[Blog de Jose Barreto](/archive/blogs/josebda/)|Blog escrito por un administrador del programa principal del equipo de Servidor de archivos de Microsoft|
|[Blog del equipo de Almacenamiento de Microsoft: File Cabinet](https://go.microsoft.com/fwlink/p/?LinkId=392740)|Blog sobre los servicios de archivo y las características de almacenamiento en Windows Server|
...
Para el acceso externo a la granja de SharePoint, puede crear un registro de host en un servidor DNS externo con la misma dirección URL que los clientes usan en la intranet (por ejemplo, `https://sharepoint.contoso.com`) que apunta a una dirección IP externa en el firewall. (Un procedimiento recomendado, usando este ejemplo, es configurar DNS dividido para que el servidor DNS interno sea autoritativo y `contoso.com` enruta las solicitudes directamente al clúster de SharePoint granja de servidores, en lugar de enrutar las solicitudes DNS al servidor DNS externo). A continuación, puede asignar la dirección IP externa a la dirección IP interna del clúster local para que los clientes encuentren los recursos que buscan.
...
|Nombre del servidor|Función|Configuración|
|---|---|---|
|DC1|Controlador de dominio con Active Directory.|Dos procesadores <br/> De 512 MB a 4 GB de RAM <br/> 1 disco duro de 127 GB|
|RRAS|Servidor configurado con el rol servicio de enrutamiento y acceso remoto (RRAS).|Dos procesadores <br/> De 2 a 8 GB de RAM <br/> 1 disco duro de 127 GB|
|FS1|Servidor de archivos con recursos compartidos para copias de seguridad y un extremo para DFSR.|Cuatro procesadores <br/> De 2 a 12 GB de RAM <br/> 1 disco duro de 127 GB <br/> Disco duro (SAN) 1 x 1 TB <br/> Disco duro 1 x 750 GB|
|SP-WFE1, SP-WFE2|Servidor front-end web|Cuatro procesadores <br/> 16 GB de RAM|
|SP-APP1, SP-APP2, SP-APP3|Servidores de aplicaciones.|Cuatro procesadores <br/> De 2 a 16 GB de RAM|
|SP-SQL-HA1, SP-SQL-HA2|Servidores de base de datos, configurados con los grupos de disponibilidad AlwaysOn de SQL Server 2012 para proporcionar alta disponibilidad. Esta configuración usa SP-SQL-HA1 y SP-SQL-HA2 como réplicas principales y secundarias.|Cuatro procesadores <br/> De 2 a 16 GB de RAM|
...
|Letra de unidad|Size|Nombre de directorio|Path|
|---|---|---|---|
|C|80|Unidad del sistema|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\|
|E|80|Unidad de registro (40 GB)|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\DATA|
|F|80|Página (36 GB)|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\MSSQL\\DATA|
...
|Letra de unidad|Size|Nombre de directorio|Path|
|---|---|---|---|
|C|80|Directorio raíz de datos|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\|
|E|500|Directorio de base de datos de usuario|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\DATA|
|F|500|Directorio de registro de base de datos de usuario|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\DATA|
|G|500|Directorio de base de datos temporal|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\DATA|
|H|500|Directorio de registro de base de datos temporal|\<DriveLetter\>:\\Archivos de programa\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\DATA|
...
> [!NOTE]
> Cree bases de datos de marcador de posición para poder crear los grupos de disponibilidad antes de instalar SharePoint. Para más información, consulte [Configuración de grupos de disponibilidad AlwaysOn de SQL Server 2012 para SharePoint 2013](/SharePoint/administration/configure-an-alwayson-availability-group).
...
Usamos el parámetro  _skipRegisterAsDistributedCachehost_ cuando ejecutamos **psconfig.exe** en la línea de comandos. Para más información, vea [Planear las fuentes y el servicio de caché distribuida en SharePoint Server 2013](/sharepoint/administration/plan-for-feeds-and-the-distributed-cache-service).
...
|Nombre del servidor|Función|Configuración|Subred|Conjunto de disponibilidad|
|---|---|---|---|---|
|spDRAD|Controlador de dominio con Active Directory|Dos procesadores <br/> De 512 MB a 4 GB de RAM <br/> 1 disco duro de 127 GB|sp-ADservers||
|AZ-SP-FS|Servidor de archivos con recursos compartidos para copias de seguridad y un extremo para DFSR|Configuración de A5: <br/> Dos procesadores <br/> 14 GB de RAM <br/> 1 disco duro de 127 GB <br/> 1 disco duro de 135 GB <br/> 1 disco duro de 127 GB <br/> 1 disco duro de 150 GB|sp-databaseservers|DATA_SET|
|AZ-WFE1, AZ -WFE2|Servidores front-end web|Configuración de A5: <br/> Dos procesadores <br/> 14 GB de RAM <br/> 1 disco duro de 127 GB|sp-webservers|WFE_SET|
|AZ -APP1, AZ -APP2, AZ -APP3|Servidores de aplicaciones|Configuración de A5: <br/> Dos procesadores <br/> 14 GB de RAM <br/> 1 disco duro de 127 GB|sp-applicationservers|APP_SET|
|AZ -SQL-HA1, AZ -SQL-HA2|Servidores de base de datos y réplicas principales y secundarias para los grupos de disponibilidad AlwaysOn|Configuración de A5: <br/> Dos procesadores <br/> 14 GB de RAM|sp-databaseservers|DATA_SET|
...
Los servicios pueden tardar varios minutos en iniciarse, en función del rendimiento y la carga actual de SharePoint Server. Haga clic manualmente en **Iniciar** para el servicio y proporcione el tiempo adecuado para el inicio mientras actualiza ocasionalmente la pantalla Servicios en el servidor para supervisar su estado. En caso de que el servicio permanezca detenido, habilite SharePoint registro de diagnóstico, intente iniciar el servicio de nuevo y, a continuación, compruebe si hay errores en el registro. Para obtener más información, consulte [Configuración del registro de diagnóstico en SharePoint 2013](/sharepoint/administration/configure-diagnostic-logging).
...
[Compatibilidad de alta disponibilidad y opciones de recuperación ante desastres para bases de datos de SharePoint](/sharepoint/administration/supported-high-availability-and-disaster-recovery-options-for-sharepoint-databas)

[Configuración de grupos de disponibilidad AlwaysOn de SQL Server 2012 para SharePoint 2013](/SharePoint/administration/configure-an-alwayson-availability-group)
