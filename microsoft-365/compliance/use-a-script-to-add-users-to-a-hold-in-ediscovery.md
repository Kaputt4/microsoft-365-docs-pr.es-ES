...
- **Consulta de búsqueda para una suspensión basada en consulta:** Puede crear una suspensión basada en consulta para que solo se conserve el contenido que cumpla los criterios de búsqueda especificados. Para poner todo el contenido en espera, simplemente presione **entrar** cuando se le pida una consulta de búsqueda.

- **Indica si se va a activar o no la suspensión:** Puede hacer que el script Active la retención después de su creación o puede hacer que la secuencia de comandos cree la retención sin habilitarla. Si la secuencia de comandos no se activa, puede activarla más adelante en el centro de seguridad & cumplimiento o mediante la ejecución de los siguientes comandos de PowerShell:
...
- **LocationsOnHold.txt:** Contiene una lista de los buzones y los sitios de OneDrive para la empresa en los que la secuencia de comandos se colocó correctamente en retención.

- **LocationsNotOnHold.txt:** Contiene una lista de buzones y sitios de OneDrive para la empresa que el script no se conserve en retención. Si un usuario tiene un buzón de correo, pero no un sitio de OneDrive para la empresa, el usuario se incluiría en la lista de sitios de OneDrive para la empresa que no se pusieron en suspensión.

- **GetCaseHoldPolicy.txt:** Contiene el resultado del cmdlet **Get-CaseHoldPolicy** para la nueva retención, que el script se ejecutó después de crear la nueva retención. La información devuelta por este cmdlet incluye una lista de usuarios cuyos buzones y los sitios de OneDrive para la empresa se pusieron en retención y si la retención está habilitada o deshabilitada. 

- **GetCaseHoldRule.txt:** Contiene el resultado del cmdlet **Get-CaseHoldRule** para la nueva retención, que el script se ejecutó después de crear la nueva retención. La información devuelta por este cmdlet incluye la consulta de búsqueda si usó el script para crear una suspensión basada en consulta.
