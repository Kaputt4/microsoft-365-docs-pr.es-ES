...
- **Teams chats 1:1**. Mensajes de chat, publicaciones y datos adjuntos compartidos en Teams conversación entre dos personas.  Teams chats 1:1 también se *denominan conversaciones*.
...
- **Teams canales**. Mensajes de chat, publicaciones, respuestas y datos adjuntos compartidos en un canal Teams estándar.

- **Canales privados**. Mensajes, respuestas y datos adjuntos compartidos en un canal Teams privado.

- **Canales compartidos**. Mensajes, respuestas y datos adjuntos compartidos en un canal Teams compartido.
...
## <a name="create-a-case-for-teams-content"></a>Crear un caso para Teams contenido
...
Para obtener más información acerca de los límites de casos aumentados, vea [Usar el nuevo formato de caso en Advanced eDiscovery](advanced-ediscovery-new-case-format.md).
...
2. En la **página Orígenes de** datos, haga clic **en Agregar origen de datosAgregue** >  **nuevos custodios**.
...
5. Siga estas directrices para agregar orígenes de datos de custodia Teams contenido. Haga **clic en** Editar para agregar una ubicación de datos.
   ...
   - **SharePoint**. Agregue el sitio SharePoint asociado con cualquier canal privado o compartido del que sea miembro el custodio para agregar (y conservar) como datos de custodia los archivos compartidos en un canal. Haga **clic en** Editar y, a continuación, agregue la dirección URL del sitio SharePoint asociado a un canal privado o compartido. Para obtener información sobre cómo localizar los canales privados y compartidos de los que es miembro un usuario, vea [eDiscovery of private and shared channels](/microsoftteams/ediscovery-investigation#ediscovery-of-private-and-shared-channels).

   - **Microsoft Teams**. Agregue los equipos de los que el custodio es miembro para agregar (y conservar) como datos de custodia todos los mensajes de canal y todos los archivos compartidos a un canal Teams cliente. Esto incluye agregar el buzón para el equipo primario de un canal compartido del que el custodio es miembro. Al hacer clic **en Editar**, el buzón y el sitio asociados con cada equipo del que es miembro el custodio se muestran en una lista. Seleccione los equipos que desea asociar al custodio. Debe seleccionar el buzón y el sitio correspondientes para cada equipo.

   > [!NOTE]
   > También puede agregar el buzón y el sitio de Teams de los que los custodios no son miembros como una ubicación de datos de custodia. Para ello, haga clic en **Editar** junto **a Exchange** y **SharePoint** y, a continuación, agregue el buzón y el sitio asociados con el equipo.
...
Después de agregar custodios al caso y conservar el contenido en orígenes de datos custodiados, el siguiente paso del flujo de trabajo es buscar contenido Teams que sea relevante para la investigación y agregarlo a un conjunto de revisión para su posterior revisión y análisis. Aunque es típico recopilar contenido de Teams junto con contenido de otros servicios de Microsoft 365, como el correo electrónico en Exchange y los documentos de SharePoint, esta sección se centrará específicamente en recopilar contenido Teams en una colección. Puede crear colecciones adicionales que recopile contenido que no Teams agregar a un conjunto de revisión.

Al recopilar contenido Teams para un caso, hay dos pasos en el flujo de trabajo:

1. **Crear una colección de borradores**.  El primer paso es crear una colección *borrador*, que es una estimación de los elementos que coinciden con los criterios de búsqueda. Puede ver información sobre los resultados que coinciden con la consulta de búsqueda, como el número total y el tamaño de los elementos encontrados, los distintos orígenes de datos donde se encontraron y las estadísticas sobre la consulta de búsqueda. También puede obtener una vista previa de un ejemplo de los elementos devueltos por la colección. Con estas estadísticas, puede cambiar la consulta de búsqueda y volver a ejecutar la colección borrador tantas veces como sea necesario para restringir los resultados hasta que esté satisfecho de que está recopilando el contenido relevante para su caso.
...
2. En la **página Colecciones**, seleccione Nueva **colecciónRecopiaStandard** > .
...
5. Seleccione uno o más custodios y, a continuación, haga clic en **Agregar**.
   ...
   - Anule la selección de la casilla en la **columna** Sitio adicional para quitar los SharePoint que contienen archivos compartidos en canales privados o compartidos. Al hacerlo, se eliminan los archivos duplicados que se adjuntaban a conversaciones de canal privado o compartido, ya que estos datos adjuntos en la nube se agregan automáticamente al conjunto de revisión cuando se confirma el borrador de colección y se agrupan en las conversaciones en las que se han compartido.

6. Si ha seguido anteriormente los pasos para agregar contenido Teams como orígenes de datos custodios, puede omitir este paso y seleccionar **Siguiente**. De lo contrario,  en la página Asistente para orígenes de datos no custodiados, puede elegir orígenes de datos que no son custodiales que contengan un contenido Teams que haya agregado al caso para buscar en la colección.
...
9. En la **página Guardar borrador o** asistente para recopilar, realice una de las siguientes acciones en función de si desea crear una colección de borradores o confirmar la colección en un conjunto de revisión.
   ...
   2. **Recopilar elementos y agregarlos a un conjunto de revisión**. Elija esta opción para ejecutar la colección y, a continuación, agregue los resultados a un conjunto de revisión. Puede agregar la colección a un conjunto de revisión nuevo o existente. Las opciones para recopilar mensajes Teams de conversación contextuales (también denominados subprocesos de *conversación) y* recopilar datos adjuntos en la nube se seleccionan de forma predeterminada y no se pueden no seleccionar. Estas opciones se aplican automáticamente debido al nuevo formato de caso que usó al crear inicialmente el caso para Teams contenido. Para obtener más información acerca de cómo confirmar colecciones en un conjunto de revisión, vea [Commit a draft collection to a review set](commit-draft-collection.md).
...
- **[Agrupación](#grouping)**. Cómo los mensajes, las publicaciones y las respuestas Teams conversaciones se agrupan y se presentan en el conjunto de revisión. Esto también incluye datos adjuntos en las conversaciones de chat que se extraen y agrupan dentro de la conversación.

- **[Subprocesos de conversación de transcripción](#transcript-conversation-threading)**. How Advanced eDiscovery determines what additional content from a conversation to collect to provide context around items that matched the collection criteria.

- **[Desduplicación](#deduplication-of-teams-content)**. Cómo Advanced eDiscovery controla el contenido Teams duplicado.

- **[Metadatos](#metadata-for-teams-content)**. Propiedades de metadatos Advanced eDiscovery agrega Teams contenido después de que se recopila y se agrega a un conjunto de revisión.
