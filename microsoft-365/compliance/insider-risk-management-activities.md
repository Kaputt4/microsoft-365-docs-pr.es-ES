...
- **Alertas totales que necesitan revisión**: se muestra el número total de alertas que necesitan revisión y evaluación, incluido un desglose por gravedad de alerta.
...
- **Gravedad baja**: las actividades e indicadores de la alerta representan un riesgo menor. Las actividades de riesgo asociadas son menores, más poco frecuentes y no se pueden realizar con otros factores de riesgo significativos.
...
- **Detección de exfiltración** acumulativa: la detección acumulativa de exfiltración analiza los registros de eventos, pero aplica un modelo que incluye la desduplicación de actividades similares para calcular el riesgo de exfiltración acumulado. Además, también puede haber una diferencia en el número de actividades que se muestran en el explorador de actividades si ha realizado cambios en la directiva o configuración existentes. Por ejemplo, si modifica dominios permitidos o no permitidos o agrega nuevas exclusiones de tipos de archivo después de crear una directiva y de que se hayan producido coincidencias de actividad, las actividades acumulativas de detección de exfiltración diferirán de los resultados antes de que cambie la directiva o la configuración. Los totales acumulados de actividad de detección de exfiltración se basan en la configuración de directiva y configuración en el momento del cálculo y no incluyen actividades antes de los cambios de directiva y configuración
...
1. **Revise el [panel de alertas](insider-risk-management-activities.md#alert-dashboard) para obtener alertas con el estado de Revisión de necesidades**. [Filtrar](insider-risk-management-activities.md#filter-alerts-on-the-alert-dashboard) por estado *de alerta* si es necesario para ayudar a localizar estos tipos de alertas.
...
- **Use características de riesgo de insider automatizadas para ayudar a descubrir las actividades de mayor riesgo**. La detección de [](insider-risk-management-policies.md#sequence-detection-preview) secuencias de administración de riesgos insider y las características acumulativas de detección de [exfiltración](insider-risk-management-policies.md#cumulative-exfiltration-detection-preview) pueden ayudarle a descubrir rápidamente los riesgos más difíciles de encontrar en su organización. Considera la posibilidad de ajustar los [refuerzos de puntuación](insider-risk-management-settings.md#indicators) de riesgo, [las exclusiones](insider-risk-management-settings.md#file-type-exclusions) de tipos de [archivo, los](insider-risk-management-settings.md#domains) dominios y la configuración mínima del umbral del [indicador para tus](insider-risk-management-settings.md#indicator-level-settings-preview) directivas.
