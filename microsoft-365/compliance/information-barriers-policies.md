...
## <a name="concepts-of-information-barrier-policies"></a>Conceptos de directivas de barrera de información
...
- Los atributos de cuenta de usuario se definen en Azure Active Directory (o Exchange Online). Estos atributos pueden incluir departamento, puesto de trabajo, ubicación, nombre de equipo y otros detalles de perfil de trabajo. 
...
- Las directivas de barrera de información determinan límites o restricciones de comunicación. Al definir directivas de barrera de información, elige entre dos tipos de directivas:
...
- La aplicación de directiva se realiza después de definir todas las directivas de barrera de información y está listo para aplicarlas en su organización.

## <a name="the-work-flow-at-a-glance"></a>El flujo de trabajo de un vistazo

| Fase | Lo que implica |
|:--------|:------------------|
| [Asegurarse de que se cumplen los requisitos previos](#prerequisites) | - Compruebe que tiene las licencias y permisos [necesarios](information-barriers.md#required-licenses-and-permissions)<br/>- Comprobar que el directorio incluye datos para segmentar usuarios<br/>- Habilitar la búsqueda de directorios con ámbito para Microsoft Teams<br/>- Asegúrese de que el registro de auditoría está activado<br/>- Asegúrese de que no hay directivas de libreta de direcciones de Exchange en su lugar<br/>- Usar PowerShell (se proporcionan ejemplos)<br/>- Proporcionar consentimiento de administrador para Microsoft Teams (se incluyen pasos) |
...
- Sin directivas de libreta de direcciones: antes de definir y aplicar directivas de barrera de información, asegúrese de que no haya directivas de libreta de direcciones de Exchange. Las barreras de información se basan en directivas de libreta de direcciones, pero los dos tipos de directivas no son compatibles. Si tiene estas directivas, asegúrese de quitar primero [las directivas de la libreta de direcciones.](/exchange/address-books/address-book-policies/remove-an-address-book-policy) Una vez habilitadas las directivas de barrera de  información y habilitada la libreta jerárquica [](/exchange/address-books/hierarchical-address-books/hierarchical-address-books) de direcciones, todos los usuarios que no estén incluidos en un segmento de barrera de información verán la libreta jerárquica de direcciones en Exchange Online.

- PowerShell: actualmente, las directivas de barrera de información se definen y administran en el Centro de seguridad de Office 365 & cumplimiento con cmdlets de PowerShell. Aunque en este artículo se proporcionan varios ejemplos, deberá familiarizarse con los cmdlets y parámetros de PowerShell. También necesitará el módulo De Azure PowerShell.
    - [Conectarse a PowerShell del Centro de seguridad y cumplimiento](/powershell/exchange/connect-to-scc-powershell)
    - [Instalar el módulo de PowerShell de Azure](/powershell/azure/install-az-ps?view=azps-2.3.2)

- Consentimiento de los administradores para las barreras de información en Microsoft Teams: cuando las directivas del IB están en su lugar, pueden quitar usuarios que no son del IB de grupos (es decir, canales de Teams, que se basan en grupos). Esta configuración ayuda a garantizar que la organización cumpla con las directivas y las normativas. Use el siguiente procedimiento para permitir que las directivas de barrera de información funcionen según lo previsto en Microsoft Teams.

   1. Requisito previo: Instalar Azure PowerShell desde [Instalar Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-az-ps).
   ...
   1. Cuando se le pida, inicie sesión con su cuenta laboral o educativa para Office 365.
...
> [!TIP]
> Para ayudarle a preparar el plan, se incluye un escenario de ejemplo en este artículo. [Vea Departamentos, segmentos](#example-contosos-departments-segments-and-policies)y directivas de Contoso.<p>Además, hay disponible un libro de Excel descargable para ayudarle a planear y definir los segmentos y directivas (y crear los cmdlets de PowerShell). [Obtener el libro](https://github.com/MicrosoftDocs/OfficeDocs-O365SecComp/raw/public/SecurityCompliance/media/InfoBarriers-PowerShellGenerator.xlsx).
...
- [Escenario 1: Bloquear comunicaciones entre segmentos](#scenario-1-block-communications-between-segments)
...
### <a name="scenario-1-block-communications-between-segments"></a>Escenario 1: Bloquear comunicaciones entre segmentos

Cuando desea bloquear segmentos para que no se comuniquen entre sí, defina dos directivas: una para cada dirección. Cada directiva bloquea la comunicación solo de un modo.
...
| Directivas de barrera de información | Use el cmdlet **Get-InformationBarrierPolicy.** <p> Sintaxis: `Get-InformationBarrierPolicy` <p> Este cmdlet mostrará una lista de directivas de barrera de información definidas y su estado. |
...
Para ver cómo una organización puede acercarse a la definición de segmentos y directivas, considere el siguiente ejemplo.

### <a name="contosos-departments-and-plan"></a>Departamento y plan de Contoso
...
Con los segmentos definidos, Contoso continúa definindo directivas.

### <a name="contosos-information-barrier-policies"></a>Directivas de barrera de información de Contoso

Contoso define tres directivas, como se describe en la tabla siguiente:

| Directiva | Definición de directiva |
|:---------|:--------------------|
| **Directiva 1: Impedir que las ventas se comuniquen con Research** | `New-InformationBarrierPolicy -Name "Sales-Research" -AssignedSegment "Sales" -SegmentsBlocked "Research" -State Inactive` <p> En este ejemplo, la directiva de barrera de información se denomina *Sales-Research*. Cuando esta directiva está activa y aplicada, ayudará a impedir que los usuarios que están en el segmento ventas se comuniquen con los usuarios del segmento investigación. Esta directiva es una directiva uni-way; no impedirá que Research se comunique con Sales. Para ello, se necesita la directiva 2. |
| **Directiva 2: Impedir que la investigación se comunique con Ventas** | `New-InformationBarrierPolicy -Name "Research-Sales" -AssignedSegment "Research" -SegmentsBlocked "Sales" -State Inactive` <p> En este ejemplo, la directiva de barrera de información se denomina *Research-Sales*. Cuando esta directiva está activa y aplicada, ayudará a impedir que los usuarios que se encuentran en el segmento investigación se comuniquen con los usuarios del segmento ventas. |
| **Directiva 3: Permitir que la fabricación se comunique solo con recursos humanos y marketing** | `New-InformationBarrierPolicy -Name "Manufacturing-HRMarketing" -AssignedSegment "Manufacturing" -SegmentsAllowed "HR","Marketing","Manufacturing" -State Inactive` <p> En este caso, la directiva de barrera de información se denomina *Manufacturing-HRMarketing*. Cuando esta directiva está activa y aplicada, la fabricación solo se puede comunicar con recursos humanos y marketing. Los recursos humanos y el marketing no están restringidos para comunicarse con otros segmentos. |
