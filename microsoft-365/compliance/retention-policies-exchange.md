...
Tanto un buzón como una carpeta pública utilizan la carpeta de Elementos recuperables para retener elementos. Sólo las personas a las que se les han asignado permisos de eDiscovery pueden ver los elementos de la carpeta Elementos recuperables de otro usuario.
  
Cuando una persona elimina un mensaje en una carpeta distinta de la carpeta Elementos eliminados, de forma predeterminada, el mensaje pasa a la carpeta Elementos eliminados. Cuando una persona elimina un elemento de la carpeta Elementos eliminados, el mensaje se mueve a la carpeta Elementos recuperables. Sin embargo, un usuario puede borrar un elemento (MAYÚS+BORRAR) en cualquier carpeta, lo que evita la carpeta Elementos eliminados y mueve el elemento directamente a la carpeta Elementos recuperables.
...
El trabajo del temporizador puede tardar hasta 7 días en ejecutarse y la ubicación de la Bolsa debe contener al menos 10 MB.
...
1. **Si el elemento no se elimina** durante el período configurado: al final del período configurado en la directiva de retención, el artículo se mueve a la carpeta de artículos recuperables. 

2. **Si el elemento se elimina** durante el período configurado: el elemento se mueve inmediatamente a la carpeta de elementos recuperables. Si un usuario elimina el elemento o vacía la carpeta elementos recuperables, el elemento se elimina de forma permanente. En caso contrario, el elemento se elimina permanentemente después de estar en la carpeta elementos recuperables durante 14 días. 
...
Para excluir los tipos seleccionados para los artículos de Exchange en una directiva de retención, utilice el `ExcludedItemClasses` parámetro con los  `New-RetentionComplianceRule` y `Set-RetentionComplianceRule` cmdlets.
...
- **Permitirme elegir ubicaciones específicas.** > **Correo electrónico de Exchange** y **Carpetas públicas de Exchange**
