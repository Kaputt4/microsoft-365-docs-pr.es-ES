---
title: Actualizar los registros MX para realizar la transición al servicio de Exchange Online global
f1.keywords:
- CSH
ms.author: pebaum
author: pebaum
manager: scotv
audience: Admin
ms.topic: article
ms.service: o365-administration
ms.localizationpriority: medium
ms.custom:
- AdminSurgePortfolio
- admindeeplinkMAC
ROBOTS: NOINDEX, NOFOLLOW
description: Obtenga información sobre cómo realizar la transición de Microsoft Cloud Germany Exchange Online al servicio de Exchange Online global
ms.openlocfilehash: cf11b44ffd8bb52151100e802929d82f5498c668
ms.sourcegitcommit: d4b867e37bf741528ded7fb289e4f6847228d2c5
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 10/06/2021
ms.locfileid: "60192660"
---
# <a name="update-your-mx-records-to-transition-to-the-global-exchange-online-service"></a>Actualizar los registros MX para realizar la transición al servicio de Exchange Online global

1. Inicie sesión en Centro de administración de Microsoft 365 y vaya a **Configuración**  >  <a href="https://go.microsoft.com/fwlink/p/?linkid=834818" target="_blank">**dominios**</a>

2. El estado se mostrará en el lado derecho de cada dominio. Si los dominios de su organización apuntan a Microsoft Cloud Germany Exchange Online, deberá actualizar el registro MX.

3. Haga clic en el dominio y, a continuación, en **Errores DNS detectados, haga clic aquí para ver**.

4. Esta página tendrá instrucciones para mostrar cómo corregir el registro MX. Si el registrador de su dominio usa [dominio](../setup/add-domain.md#registrars-with-domain-connect)Conectar , puede hacer clic en "Corregir mis registros" en la parte superior. De lo contrario, puede seguir el vínculo del asistente para obtener instrucciones **paso a paso** para el registrador.