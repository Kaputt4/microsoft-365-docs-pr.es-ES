---
title: Configuración de la integración de compatibilidad de Microsoft 365 con Azure AD token de autenticación
f1.keywords:
- NOCSH
ms.author: efrene
author: efrene
...
---
# <a name="configure-microsoft-365-support-integration-with-azure-ad-auth-token"></a>Configuración de la integración de compatibilidad de Microsoft 365 con Azure AD token de autenticación
...
1. \[\] administrador de AAD Cree Azure AD Application for Outbound en el inquilino de Microsoft 365.
...
1. \[\] administrador de AAD Cree una API de Azure AD Application for Rest en el inquilino de Microsoft 365.
...
1. \[\] administrador de AAD Cree una aplicación de Azure AD para el usuario rest en el inquilino de Microsoft 365.
...
1. Cree una nueva aplicación con la opción **Conectar a un proveedor de OAuth de terceros** y escriba estos valores:

    - Id. de cliente: este es el identificador de cliente de la aplicación creada en el paso \#1 de Requisitos previos (Azure AD token de autenticación).
...
1. \[Administrador de\] ServiceNow Para configurar el proveedor OIDC en ServiceNow, consulte la [documentación en línea](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/security/task/add-OIDC-entity.html).
...
1. Seleccione **Nuevo** y, a continuación, **seleccione Crear nuevo proveedor de Conectar open id**.

1. En **Configuración del proveedor OIDC de OAuth**, seleccione **Buscar** y cree una nueva configuración de proveedor OIDC en **oidcproviderconfiguration.list\_\_** con estos valores:

    - Proveedor OIDC: **Azure {TenantName\_}** (por ejemplo: Contoso Azure)
...
1. Para crear una aplicación, seleccione **Configurar un proveedor OIDC para comprobar los tokens de identificador** con estos valores:
    ...
    - Id. de cliente: id. de cliente de la aplicación creada en requisitos previos (Azure AD token de autenticación) paso \#3.

    - Secreto de cliente: el secreto de aplicación de la aplicación creada en requisitos previos (Azure AD token de autenticación) paso \#3.
...
1. \[Administrador de\] ServiceNow Crear usuarios de integración.
    ...
    Si va a crear un nuevo usuario de integración, active la opción **Solo acceso al servicio web** . También debe conceder a este usuario el rol **incidentmanager\_**.
...
Si su empresa limita el acceso a Internet con sus propias directivas, habilite el acceso de red para el servicio de Microsoft 365 integración de soporte técnico al permitir las direcciones IP siguientes para el acceso de API entrante y saliente.
...
## <a name="configure-the-microsoft-365-support-integration-application"></a>Configuración de la aplicación de integración de compatibilidad con Microsoft 365

La aplicación de integración de compatibilidad con Microsoft 365 se puede configurar en Microsoft 365 soporte técnico.

Estos pasos son necesarios para configurar la integración entre la instancia de ServiceNow y Microsoft 365 soporte técnico.
...
1. \[Administrador de\] ServiceNow Vaya al **programa de instalación de soporte técnico &gt; de Microsoft 365** para abrir el flujo de trabajo de integración.

    > [!NOTE]
    > Si ve el error "Read operation against 'oauthentity\_' from scope 'xmiomsm365assis\_\_\_' has been refused due to the table's cross-scope access policy", it was caused by your table access policy. Debe asegurarse de que **todos los ámbitos de aplicación que se pueden leer estén comprobados &gt;** para la oauthentity\_ de la tabla.
...
1. \[Administrador de\] ServiceNow Seleccione **Aceptar** la solicitud de consentimiento para continuar.
...
1. \[ServiceNow Admin\] Configure el entorno y el tipo de instalación.
...
1. \[Administrador de\] ServiceNow Escriba el dominio de inquilino de Microsoft 365.
...
1. \[ServiceNow Admin\] Configure Inbound settings (Configuración de entrada).
    1. Configure la aplicación de AAD de entrada.
    ...
    1. Configure el proveedor de Conectar OpenID externo de ServiceNow (proveedor OIDC).
...
1. \[administrador de inquilinos\] de Microsoft 365 Complete la integración.
    ...
    - Tipo de autenticación: seleccione **AAD autenticación**.
    ...
    - Nombre de usuario de rest: valor de **Nombre de usuario** del paso 6: completar la integración, que es el **identificador de cliente** de la aplicación creada en el paso \#3 de Requisitos previos (Azure AD token de autenticación).

    - Contraseña de usuario rest: el secreto de aplicación de la aplicación que se creó en requisitos previos (Azure AD token de autenticación) paso \#3.
   ...
   :::image type="content" source="../../media/ServiceNow-guide/snowaadoauth-10.png" lightbox="../../media/ServiceNow-guide/snowaadoauth-10.png" alt-text="Interfaz gráfica de usuario, texto, aplicación, descripción de correo electrónico generada automáticamente":::
    La Microsoft 365 aplicación de integración de soporte técnico ejecutará pruebas para asegurarse de que la integración funciona. Si hay un problema con la configuración, un mensaje de error explicará lo que se debe corregir. De lo contrario, la aplicación está lista.
...
1. \[Administrador de\] ServiceNow Habilite la integración de soporte técnico de Microsoft para un usuario existente.
    ...
    Microsoft 365 integración de soporte técnico está habilitada para el usuario con uno de estos roles:

    - xmiomsm365assis.insightsuser\_\_\_\_

    - xmiomsm365assis.administrator\_\_\_

1. \[OPCIONAL\] \[El usuario con el rol xmiomsm365assis.administrator\_\_\_ link\] Link Microsoft 365 cuenta de administrador.

    Si algún usuario tiene el rol xmiomsm365assis.administrator\_\_\_ y usa cuentas de Microsoft 365 diferentes para administrar un caso de soporte técnico de Microsoft 365, debe ir a Microsoft 365 cuenta de vínculo de soporte técnico &gt; para configurar su correo electrónico de administrador de Microsoft 365.
