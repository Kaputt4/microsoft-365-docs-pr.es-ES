...
|4.  <br/> |La vista **archivos** muestra el número de archivos totales y activos. Un archivo se considera activo si se guardó, sincronizó, modificó o compartió en un período de tiempo específico.  <br/> Nota: una actividad de archivo se puede producir varias veces para un solo archivo, pero solo se contará como un archivo activo. Por ejemplo, puede guardar y sincronizar el mismo archivo varias veces durante un período de tiempo especificado, pero solo contará como un único archivo activo y como un único archivo sincronizado en los datos.           |
|5.  <br/> |La vista **Almacenamiento** muestra la tendencia de la cantidad de almacenamiento que esté usando en OneDrive. Si desea comprobar los límites de almacenamiento, consulte [comprobar si un usuario tiene el límite de almacenamiento predeterminado o un límite específico](https://docs.microsoft.com/onedrive/set-default-storage-space#check-if-a-user-has-the-default-storage-limit-or-a-specific-limit).  <br/> Nota: el tamaño incluye todas las versiones y los metadatos asociados con los archivos.           |
