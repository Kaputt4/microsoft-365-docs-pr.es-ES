...
Si es un cliente de Microsoft 365 con buzones en Exchange Online o un cliente independiente de Exchange Online Protection (EOP) sin buzones de Exchange Online, EOP ofrece varias formas de garantizar que los usuarios reciban correo electrónico de remitentes de confianza. Estas opciones incluyen Exchange de flujo de correo (también conocidas como reglas de transporte), remitentes de Outlook Caja fuerte, la lista de direcciones IP permitidas (filtrado de conexiones) y listas de remitentes permitidos o listas de dominios permitidos en directivas contra correo no deseado. Colectivamente, puede pensar en estas opciones como _listas de remitentes seguros_.
...
> [!IMPORTANT]
>
> - Los mensajes que se identifican como malware o phishing de elevada confianza siempre se ponen en cuarentena, independientemente de la opción de lista de remitentes seguros que use. Para obtener más información, [vea Proteger de forma predeterminada en Office 365](secure-by-default.md).
>
> - Tenga cuidado de supervisar detenidamente _las excepciones_ que realice al filtrado de correo no deseado mediante listas de remitentes seguros.
>
...
>
> - Para permitir que un dominio envíe correo electrónico no autenticado (omitir la protección contra la suplantación de identidad) pero no omitir el correo no deseado y otras protecciones, puede [](learn-about-spoof-intelligence.md) usar la información de inteligencia suplantada y la lista de inquilinos permitidos [o](tenant-allow-block-list.md) bloqueados.
>
...
En cambio, también tiene varias opciones para bloquear el correo electrónico de orígenes específicos mediante _listas de remitentes bloqueados_. Para más información, consulte [Crear listas de remitentes bloqueados en EOP](create-block-sender-lists-in-office-365.md).
...
En el siguiente ejemplo se supone que necesita correo electrónico de contoso.com omitir el filtrado de correo no deseado. Para ello, configure las siguientes opciones:
...
3. **Condiciones opcionales**:
   ...
   - **El asunto o el cuerpo** \> **asunto o cuerpo incluye cualquiera de estas palabras** \> \<keywords\>: Si puede restringir aún más los mensajes por palabras clave o frases en la línea de asunto o el cuerpo del mensaje, puede usar esas palabras como condición.
...
4. **Acción**: configure ambas acciones en la regla:
   ...
   2. **Modificar las propiedades del mensaje** \> **establecer un encabezado de mensaje**: **Establezca el encabezado del mensaje** \<CustomHeaderName\> **en el valor** \<CustomHeaderValue\>.
...
- El `5322.From` (también conocido como la dirección **De** o el remitente P2) es la dirección de correo  electrónico en el campo De encabezado y es la dirección de correo electrónico del remitente que se muestra en los clientes de correo electrónico.

Con frecuencia, las `5321.MailFrom` direcciones y `5322.From` son las mismas (comunicación de persona a persona). Sin embargo, cuando el correo electrónico se envía en nombre de otra persona, las direcciones pueden ser diferentes. Esto sucede con mayor frecuencia en los mensajes de correo electrónico masivos.
...
Caja fuerte listas de remitentes y listas de dominios seguros en directivas contra correo no deseado en EOP `5322.From` inspeccionan solo las direcciones, esto es similar a Outlook Caja fuerte remitentes que usan la `5322.From` dirección.
...
- [Use una regla de flujo](#recommended-use-mail-flow-rules) de correo con una condición que busca mensajes de blueyonder@news.blueyonderairlines.com ( `5322.From` la dirección, blueyonder.airlines@margiestravel.com ( `5321.MailFrom`la ), o ambas.
