...
Las directivas para configurar la protección contra suplantación de identidad (phishing) están disponibles en Microsoft 365 organizaciones con buzones de correo de Exchange Online, organizaciones independientes de Exchange Online Protection (EOP) sin buzones de Exchange Online y organizaciones de Microsoft defender para Office 365.

Las directivas antiphishing en Microsoft defender para Office 365 solo están disponibles en las organizaciones que tienen defender para Office 365. Por ejemplo:
...
- [Microsoft defender para Office 365 como un complemento](https://products.office.com/exchange/advance-threat-protection)

En la tabla siguiente se describen las diferencias de alto nivel entre las directivas antiphishing en EOP y las directivas antiphishing en Microsoft defender para Office 365:
...
En el resto de este artículo se describe la configuración disponible en anti-phishing policies in EOP and defender for Office 365.

## <a name="policy-settings"></a>Configuración de directivas

Las siguientes opciones de configuración de directivas están disponibles en anti-phishing policies in EOP y Microsoft defender para Office 365:
...
La siguiente configuración de suplantación de identidad está disponible en anti-phishing policies in EOP y Microsoft defender para Office 365:
...
La identificación de remitente sin autenticar forma parte de la [configuración de suplantación](#spoof-settings) de identidad disponible en las directivas antiphishing en EOP y Microsoft defender para Office 365, tal como se describe en la sección anterior.
...
## <a name="exclusive-settings-in-anti-phishing-policies-in-microsoft-defender-for-office-365"></a>Configuración exclusiva en directivas antiphishing en Microsoft defender para Office 365

En esta sección se describen las opciones de directiva que solo están disponibles en las directivas antiphishing en Microsoft defender para Office 365.

> [!NOTE]
> De forma predeterminada, la configuración exclusiva de defender para Office 365 no está configurada ni activada, incluso en la directiva predeterminada. Para aprovechar estas características, debe habilitarlas y configurarlas en la Directiva de suplantación de identidad (phishing) predeterminada, o bien crear y configurar directivas antiphishing personalizadas.
...
La siguiente configuración de suplantación solo está disponible en las directivas antiphishing en Microsoft defender para Office 365:

- **Usuarios que se deben proteger** : impide que las direcciones de correo electrónico internas o externas especificadas se suplanten **como remitentes de mensajes**. Por ejemplo, recibirá un mensaje de correo electrónico del Vicepresidente de su empresa que le pedirá que le envíe información interna de la empresa. ¿Lo haría? Muchas personas enviarían la respuesta sin pensar.
...
### <a name="advanced-phishing-thresholds-in-anti-phishing-policies-in-microsoft-defender-for-office-365"></a>Umbrales de suplantación de identidad avanzada en directivas antiphishing en Microsoft defender para Office 365

Los siguientes umbrales de suplantación de identidad avanzada solo están disponibles en las directivas antiphishing en Microsoft defender para Office 365. Estos umbrales controlan la sensibilidad de la aplicación de modelos de aprendizaje automáticos a los mensajes para determinar un veredicto de suplantación de identidad:
...
La probabilidad de falsos positivos (mensajes buenos marcados como no válidos) aumenta a medida que se aumenta esta configuración. Para obtener información acerca de la configuración recomendada, consulte [anti-phishing Policy in Microsoft defender for Office 365 Settings](recommended-settings-for-eop-and-office365-atp.md#anti-phishing-policy-settings-in-microsoft-defender-for-office-365).
