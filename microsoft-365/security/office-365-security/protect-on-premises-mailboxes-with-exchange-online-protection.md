...
Incluso si planea hospedar algunos o todos los buzones de correo localmente, aún puede proteger los buzones con Exchange Online Protection (EOP). Para configurar conectores, su cuenta debe ser un administrador global o un administrador de la compañía de Exchange (el grupo de roles Administración de la organización). Para obtener información sobre cómo se relacionan los permisos de Office 365 con los permisos de Exchange, vea Asignar roles de administrador en [Office 365 operado por 21Vianet.](https://docs.microsoft.com/microsoft-365/admin/add-users/assign-admin-roles?view=o365-21vianet&preserve-view=true) Si todos los buzones de Exchange son locales, siga estos pasos para configurar el servicio EOP.

## <a name="step-1-use-the-microsoft-365-admin-center-to-add-and-verify-your-domain"></a>Paso 1: Usar el Centro de administración de Microsoft 365 para agregar y comprobar su dominio

1. En el Centro de administración de Microsoft 365, vaya a Configuración para agregar su dominio al servicio.

2. Siga los pasos del portal para agregar los registros DNS aplicables a su proveedor de host DNS con el fin de comprobar la propiedad del dominio.

> [!TIP]
> Agregar el dominio y los usuarios a Office 365 operado por [21Vianet](https://docs.microsoft.com/microsoft-365/admin/setup/add-domain?view=o365-21vianet&preserve-view=true) y crear registros DNS para [Office 365](https://docs.microsoft.com/microsoft-365/admin/services-in-china/create-dns-records-when-you-manage-your-dns-records?view=o365-21vianet&preserve-view=true) al administrar los registros DNS son recursos útiles a los que hacer referencia al agregar el dominio al servicio y configurar DNS.
...
Si tiene buzones de Exchange locales y uno o más buzones en la nube en Exchange Online, tiene una *configuración* híbrida. En una configuración híbrida, las características como el uso compartido de calendarios de disponibilidad y el enrutamiento de correo funcionan conjuntamente en los entornos locales y en la nube. Es posible que tenga una configuración híbrida mientras realiza la transición de buzones a Exchange Online. Un entorno híbrido se configura de forma diferente que la protección independiente de EOP.

Puede elegir un escenario híbrido para aprovechar el correo electrónico basado en la nube para la mayoría de los empleados. Puede hacerlo a la vez que hospeda algunos buzones de correo localmente; por ejemplo, para su departamento legal.

Una configuración híbrida puede ser compleja, pero tiene muchas ventajas. Para obtener más información sobre cómo configurar escenarios híbridos con Exchange, [consulte Exchange Server implementaciones híbridas.](https://docs.microsoft.com/Exchange/exchange-hybrid)
