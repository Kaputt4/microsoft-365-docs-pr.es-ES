...
---
# <a name="manage-the-sources-for-microsoft-defender-antivirus-protection-updates"></a>Administrar los orígenes de las actualizaciones de protección antivirus de Microsoft Defender
...
|Ubicación | Escenario de ejemplo |
|---|---|
|Windows Server Update Service | Estás usando el Servicio de actualización de Windows Server para administrar las actualizaciones de la red.|
|Microsoft Update | Desea que los puntos de conexión se conecten directamente a Microsoft Update. Esto puede ser útil para los puntos de conexión que se conectan de forma irregular a la red empresarial, o si no usas windows server update service para administrar las actualizaciones.|
...
|Actualizaciones de inteligencia de seguridad para Antivirus de Microsoft Defender y otros antimalware de Microsoft (anteriormente denominado MMPC) |[Asegúrese de que los dispositivos estén actualizados para admitir SHA-2](https://support.microsoft.com/help/4472027/2019-sha-2-code-signing-support-requirement-for-windows-and-wsus). Las actualizaciones de inteligencia de seguridad de Antivirus De Microsoft Defender se entregan a través de Windows Update y, a partir del lunes 21 de octubre de 2019, las actualizaciones de inteligencia de seguridad se firmarán exclusivamente con SHA-2. <br/>Descargue las actualizaciones de protección más recientes debido a una infección reciente o para ayudar a aprovisionar una imagen base sólida para la [implementación de VDI](deployment-vdi-microsoft-defender-antivirus.md). Por lo general, esta opción solo se debe usar como origen final de reserva y no como origen principal. Solo se usará si las actualizaciones no se pueden descargar desde windows server update service o Microsoft Update [durante un número especificado de días](/windows/threat-protection/microsoft-defender-antivirus/manage-outdated-endpoints-microsoft-defender-antivirus#set-the-number-of-days-before-protection-is-reported-as-out-of-date).|

Puede administrar el orden en que se usan los orígenes de actualización con la directiva de grupo, Microsoft Endpoint Configuration Manager, los cmdlets de PowerShell y WMI.

> [!IMPORTANT]
> Si estableces Windows Server Update Service como una ubicación de descarga, debes aprobar las actualizaciones, independientemente de la herramienta de administración que uses para especificar la ubicación. Puedes configurar una regla de aprobación automática con windows Server Update Service, que puede ser útil a medida que las actualizaciones llegan al menos una vez al día. Para obtener más información, consulta Sincronizar actualizaciones [de protección de puntos de conexión en el servicio independiente de Windows Server Update](/configmgr/protect/deploy-use/endpoint-definitions-wsus#to-synchronize-endpoint-protection-definition-updates-in-standalone-wsus).
...
4. Expande el árbol a **componentes de Windows > Windows Defender > de firma y** configura las siguientes opciones:
...
Consulta [Configurar actualizaciones de inteligencia de seguridad para Endpoint Protection](/configmgr/protect/deploy-use/endpoint-definition-updates) para obtener más información sobre cómo configurar Microsoft Endpoint Manager (rama actual).
...
- [Usar cmdlets de PowerShell para configurar y ejecutar Antivirus de Microsoft Defender](use-powershell-cmdlets-microsoft-defender-antivirus.md)
- [Cmdlets de Defender](/powershell/module/defender/index)

## <a name="use-windows-management-instruction-wmi-to-manage-the-update-location"></a>Usar Windows Management Instruction (WMI) para administrar la ubicación de actualización
...
8. Use la línea de comandos para configurar la tarea programada.
   ...
   - Para x86 completo:
    ...
    > [!NOTE]
    > Cuando se crean las tareas programadas, puedes encontrar estas en el Programador de tareas en Microsoft\Windows\Windows Defender
