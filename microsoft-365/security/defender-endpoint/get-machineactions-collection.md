...
description: Obtenga información sobre cómo usar la API List MachineActions para recuperar una colección de acciones de máquina en Microsoft Defender para punto de conexión.
keywords: apis, graph api, api admitidas, colección machineaction
...
La consulta de `$filter` OData se admite en las propiedades , `status``id`, `machineId`, `type`, `requestor`y `creationDateTimeUtc` .
...
Si se ejecuta correctamente, este método devuelve el código de respuesta 200, Ok con una colección de entidades [machineAction](machineaction.md) .
...
Este es un ejemplo de una solicitud que filtra MachineActions por identificador de equipo y muestra las dos últimas MachineActions.
