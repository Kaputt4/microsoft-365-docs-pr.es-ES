...
>[!NOTE]
>Se agregará compatibilidad con entidades adicionales a medida que se agregan nuevas tablas al [esquema de búsqueda avanzado](advanced-hunting-schema-tables.md).

Las consultas simples, como las `project` que no usan el operador or `summarize` para personalizar o agregar resultados, normalmente devuelven estas columnas comunes.

Hay varias maneras de garantizar que las consultas más complejas devuelvan estas columnas. Por ejemplo, si `DeviceId`prefiere agregar y contar por entidad debajo de una columna como , `Timestamp` `ReportId` todavía puede devolver y obtenerlo del evento más reciente que implica cada único `DeviceId`.
...
> [!TIP]
> Para obtener un mejor rendimiento de consulta, establezca un filtro de tiempo que coincida con la frecuencia de ejecución prevista para la regla. Dado que la ejecución menos frecuente es _cada 24 horas_, el filtrado del último día cubrirá todos los datos nuevos.
...
Cuando se selecciona, puede optar por aplicar la acción Archivo **de** `SHA1`cuarentena en los archivos de la columna , `InitiatingProcessSHA1`, `SHA256`o de `InitiatingProcessSHA256` los resultados de la consulta. Esta acción elimina el archivo de su ubicación actual y coloca una copia en cuarentena.
...
Para ver todas las reglas de detección personalizadas existentes, vaya a **Reglas de** detección **HuntingCustom** > . La página enumera todas las reglas con la siguiente información de la ejecución:
...
En la pantalla detalles de la regla (**detecciones HuntingCustom** > **[Nombre** de regla]), ve a **Alertas** desencadenadas, que enumera las alertas **generadas por coincidencias** >  con la regla. Seleccione una alerta para ver la información detallada al respecto y realizar las siguientes acciones:
...
En la pantalla detalles de la regla (**detecciones HuntingCustom** > **[Nombre** de regla]), vaya a **Acciones** desencadenadas, que enumera las acciones realizadas en función de las **coincidencias** >  con la regla.
